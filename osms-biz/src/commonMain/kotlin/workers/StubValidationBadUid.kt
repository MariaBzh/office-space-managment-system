...
fun ICorChainDsl<OsmsContext>.stubValidationBadUid(title: String) = worker {
    this.title = title
    on { stubCase == OsmsStub.BAD_UID && state == OsmsState.RUNNING }
    handle {
        state = OsmsState.FAILING
        this.errors.add(
            OsmsBookingStub.getError(OsmsStub.BAD_UID.name)
        )
    }
}
